...
public:
	...
	virtual ToveShaderData *getData() { return nullptr; }

	virtual TovePaintColorAllocation getColorAllocation() const { return TovePaintColorAllocation{0, 0, 0}; }
...
public:
	ColorFeed(const GraphicsRef &graphics, float scale) :
		...
		ERR_FAIL_COND(feeds.size() != paints->getNumPaints());
	}

	TovePaintColorAllocation getColorAllocation() const {
		ERR_FAIL_COND_V(paints->getNumPaints() != graphics->getPaintIndices()->getNumPaints(), TovePaintColorAllocation());
